...
1. Wejdź do katalogu domeny `hornetcut.pl`, zwykle `public_html`.
...
Po wgraniu `https://hornetcut.pl/` powinno działać od razu, razem z `/dane-techniczne/`
...
1. hPanel → **E-maile → Konta e-mail** → utwórz `formularz@hornetcut.pl`.
...
- Rekordy A domeny `hornetcut.pl` mają wskazywać na serwer Hostingera. Jeżeli domena
...
- [ ] Google Search Console: dodaj własność `https://hornetcut.pl`, prześlij `sitemap-index.xml`.
- [ ] Sprawdź `https://hornetcut.pl/robots.txt` i `https://hornetcut.pl/llms.txt`.
- [ ] Sprawdź podgląd linku w Messengerze albo LinkedIn (obrazek `og/hornetcut.png`).
